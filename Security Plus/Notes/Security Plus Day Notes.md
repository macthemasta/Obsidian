...
- Certificate pinning is when an application has hard-coded the server’s certificate into the application itself. An application which pins a certificate or public key no longer needs to depend on others - such as DNS or CAs = when making security decisions relating to a peer’s identity.

  Certificate Stapling --> Stapling - The process of appending a digitally signed OCSP response to a certificate. It reduces overall OCSP traffic sent to a CA.
